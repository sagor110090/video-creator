...
<template>
    ...
    <div class="min-h-screen bg-[#f8fafc] dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
        <!-- Navigation -->
        <nav class="sticky top-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 py-4 mb-8">
            <div class="container mx-auto px-4 max-w-6xl flex justify-between items-center">
                <div class="flex items-center space-x-2">
                    ...
                    <span class="text-xl font-bold tracking-tight text-slate-800 dark:text-white">Video<span class="text-indigo-600">AI</span></span>
                </div>
                <div class="flex items-center space-x-4">
                    <!-- Theme Switcher -->
                    <button @click="toggleDarkMode" class="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                        <svg v-if="isDark" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707M16.243 17.243l.707.707M7.757 7.757l.707-.707M12 7a5 5 0 100 10 5 5 0 000-10z"></path></svg>
                    ...
                    <div v-if="channels.length > 0" class="hidden md:flex items-center -space-x-2 mr-4">
                        <img v-for="channel in channels" :key="channel.id" :src="channel.channel_thumbnail" :title="channel.channel_title" class="w-8 h-8 rounded-full border-2 border-white dark:border-slate-800 ring-1 ring-slate-100 dark:ring-slate-700 cursor-help transition-transform hover:scale-110 hover:z-10">
                    </div>
                    <a href="/youtube/auth" class="flex items-center space-x-2 bg-slate-900 dark:bg-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm">
